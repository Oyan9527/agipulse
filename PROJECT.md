...
- **自动化测试 + CI**：86 项测试覆盖过滤、合并、安全校验、精选门控、话题聚合等核心逻辑；每次提交与每轮抓取前自动运行，坏逻辑无法把脏数据写进站点。
...
**Engineering quality** — 86 automated tests (filtering, merging, security, quality-gate, aggregation) run on every push and before every fetch; silent-failure alerts flag the run red if sources or scoring break; and a Content-Security-Policy + URL allowlist + output escaping defend against injection from third-party sources.
