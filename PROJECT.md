...
- **自动化测试 + CI**：132 项测试覆盖过滤、去重、合并、安全校验、精选门控（含分类占比配额）、首次发现时间、话题聚合、信源健康检查等核心逻辑；每次提交与每轮抓取前自动运行，坏逻辑无法把脏数据写进站点。
...
**Engineering quality** — 132 automated tests (filtering, merging, security, quality-gate, aggregation) run on every push and before every fetch; silent-failure alerts flag the run red if sources or scoring break; and a Content-Security-Policy + URL allowlist + output escaping defend against injection from third-party sources.
