...
流水线每天夜间跑 1 次（排在 DeepSeek 优惠时段内，失败才补跑一次，见 pipeline.yml），
处理窗口是 48 小时——同一条目在被淘汰出窗口前，会被连续抓到最多约 2 次（1 次/天 × 2 天）。
如果每次都重新调用 prefilter+score，就是给同一条内容重复打分，属于纯浪费。
（跑得越稀，缓存省下的绝对量越小，但"同一条内容跨天不重复计费"这条保证一直需要。）
