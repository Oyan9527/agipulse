...
- 可见性选 **Public**（Public 仓库才有免费无限的 Actions 分钟数；Private 每月只有 2000 分钟额度，定时跑很快就会用完）
...
- workflow 每天只跑 1 次（UTC 17:00 = 北京时间凌晨 1 点），无需人工干预；每次有新数据才会产生提交，没有新数据就静默跳过。
- **失败自动补跑**：UTC 21:00（北京 5 点）还有一次兜底唤醒，但它会先查当天主运行的结果——已经成功就直接跳过、不花钱；只有主运行失败、被取消、或因 GitHub 高负载压根没触发时才真正补跑一次。判据用 Actions 运行记录而非"今天有没有数据提交"，因为流水线成功但当轮无新内容时本就不提交，用提交判断会把这种正常情况误当失败。
- 两个时间点都落在 **DeepSeek 优惠时段**（北京 00:30–08:30 / UTC 16:30–00:30，API 约半价）内——注意 GitHub Actions cron 用的是 UTC，见 `.github/workflows/pipeline.yml` 注释。
...
- **调抓取频率**：改 `.github/workflows/pipeline.yml` 里的 cron（比如 `"0 */2 * * *"` 是每 2 小时一次，更新更快但 DeepSeek 成本翻倍）。
...
- DeepSeek API：按量计费，且调用全部排在**优惠时段**（北京 00:30–08:30，chat 约半价、reasoner 折扣更多）。以约 700 条/天、每天仅 1 次估算，配合两级过滤（粗筛丢弃大部分噪声后才进入更贵的打分调用）+ 优惠时段折扣，日均成本预计在几分钱人民币的量级。
- **结果缓存**（`docs/data/llm-cache.json`）进一步压低成本：处理窗口是 48 小时，同一条内容在被淘汰出窗口前会被连续抓到多次（每天 1 次，48h 内最多约 2 次）；缓存把每条内容的判定结果（prefilter拒绝 / 打分完成）按内容id持久化下来，命中过的下一轮直接复用、不再调用DeepSeek，只有真正首次出现的新内容才会真正花钱调用两级判定。缓存随数据一起提交进仓库，跟着GitHub Actions的每次全新checkout生效，超出48h窗口的旧条目会自动从缓存清理，不会无限增长。
